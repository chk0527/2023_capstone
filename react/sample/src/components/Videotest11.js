...
const Videotest11 = ({ route, navigation }) => { //@1-const명 수정
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://3.86.244.172:8080/video11`); //서버get수정 @서버get수정, 서버 쿼리문 수정
  ...
  const renderRow = (rowData, index) => (
    <Row
      data={[rowData.id.toString() - 7387, rowData.timestamp, rowData.object.toString(), rowData.ava_label]} //@id순번 빼주기
      style={styles.cell}
      textStyle={styles.text}
      flexArr={flexArr}
      borderColor='white'
...
export default Videotest11; //@const명 수정